...
(defmethod print-pathname ((task has-pathname-mixin) stream &optional relative-to)
  (clim:format-textual-list (path-name task relative-to) #'princ
			    :stream stream
			    :separator ".")
  )
...
(defclass core-task-mixin (has-display-tick-mixin has-pathname-mixin has-task-type-mixin simulation-support-mixin)
...
(defclass final-output (final-object-mixin input-side-mixin core-task-mixin)
  ())
...
(defclass state-sink (has-state-variable-mixin final-object-mixin input-side-mixin core-task-mixin)
  ())
...
(defclass state-source (has-state-variable-mixin input-side-mixin output-side-mixin core-task-mixin has-properties-mixin)
  ())
...
(defmethod add-entry-point ((parent has-children-mixin) name join-name &optional port-names-and-type-constraints)
  (let* ((entry-point (make-instance 'entry-point :name name :superior parent :task-type 'entry-point :join-name join-name)))
    (loop for (port-name type-constraint) in port-names-and-type-constraints
	for port = (add-port 'output port-name entry-point)
	do (add-port-type-description port type-constraint))
...
(defmethod add-exit-point ((parent has-children-mixin) name branch-name &optional port-names-and-type-constraints)
  (let* ((exit-point (make-instance 'exit-point :name name :superior parent :task-type 'exit-point :branch-name branch-name)))
    (loop for (port-name type-constraint) in port-names-and-type-constraints
	for port = (add-port 'input port-name exit-point)
	do (add-port-type-description port type-constraint))
  ...
  (cond
   ...
   (t (with-atomic-action 
	  (let* ((new-task (apply #'call-next-method role-name task-type parent properties))
	    ...
	    (clim-utils:with-keywords-removed (new-properties actual-properties '(:dont-expand))
...
(defgeneric build-task-envelope (task-type task &key &allow-other-keys))

(defmethod build-task-envelope ((task-type t) (task t) &key &allow-other-keys) (values))
...
(defmethod rebuild-task-for-new-type ((new-task-type symbol) (task core-task-mixin) &rest properties)
  ...
  (apply #'build-task-envelope new-task-type task properties)
...
(defmacro deftask (type-name 
		   &rest stuff
		   &key implementation interface super-types primitive parameters other-assertions declarations bindings)
  (declare (ignore implementation interface primitive parameters other-assertions declarations bindings))
...
(defun defstuff-internal (type-name 
			  &key procedure implementation interface primitive ;only for procedure types
  ...
  (destructuring-bind (&key components dataflows control-flows) implementation
    (let* ((inputs (rest (assoc :inputs interface)))
	   ...
	   (task-type (cond (components 'composite-task)
			    ...
			    ((or branches (is-inherited-branching-task super-types)) (if primitive 'primitive-branching-task 'branching-task))
			    (t 'task-interface)))
	 ...
	 ,@(when procedure
		 ...
		 (defmethod create-task (role-name (task-type (eql ',type-name)) parent &rest properties &key ,@all-parameters &allow-other-keys)
		   ...
		   (let ((the-task (make-instance ',task-type
		     ...
		     (apply #'build-task-envelope task-type the-task properties))))
		 (defmethod build-task-envelope ((task-type (eql ',type-name)) (the-task ,task-type) &rest properties 
						 &key ,@all-parameters
						 &allow-other-keys)
		   ;; :add-type-descriptions? t below means that if the parent has already provided a port
		   ;; just add typing information
		   ;; Inheritance of envelope description from parent types
		   ,@declarations
		   (declare (ignorable ,@all-parameters))
		   (let* ,(process-deftask-bindings bindings)
		     (declare (ignorable ,@bindings-names))
		   ,@(when super-types
			   ...
			   collect `(apply #'build-task-envelope ',super-type the-task properties)))
  ...
  (let* ((real-name (if (symbolp description) description (first description)))
    ...
    (values 
	 ...
	 `(add-port ',direction ',real-name ,parent))
       (list*
	`(tell [,predicate ,task-type ,(or branch-name direction) ,real-name])
	(when type-constraint
