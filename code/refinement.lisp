...
(defun implement-plan (design-editor parent plan-name args choices)
  (let* ((unique-plan-name (intern (clim-utils:gensymbol (name parent) "IMPLEMENTATION")))
...
(defparameter *talk-to-me-about-implementations* t)
		 ...
		 (cond
		  ...
		  (t (setq selected-implementation 
			   ...
			   (choose-selected-implementation 
			    ...
			    (clim:get-frame-pane clim:*application-frame* 'learned-facts))
  ...
  (let ((answers nil))
    (flet ((process-one-level (sub-task history implementation)
		 ...
		 (loop for source in state-sources
		     ...
		     for code = (when init (initialization-code-for (task-type init) init))
		     for locality = (getf (properties source) :locality)
		     do (push (list source source-name code locality) answers))))))
      (map-over-selected-implementation task #'process-one-level))
    answers))

(defmethod initialization-code-for ((type (eql 'allocate)) initializing-task)
  (let* ((port (first (outputs initializing-task)))
    ...
    (or (allocation-code type-constraint)
	(allocation-code type-constraint-definition))
    ))
...
(defun choose-selected-implementation (parent-design name possible-implementations learned-stuff-pane)
  (declare (ignore name))
	  ...
	  (destructuring-bind (implementation action object verb verb-modifier) answer
	    ...
	    (let ((answer (get-response)))
	      ...
	      (with-open-file (stream "natsoft:code;learned-stuff.lisp" :direction :output 
			       ...
			       :if-exists :append)
		(print `[reason-for ,(full-symbolic-description implementation) ,(full-symbolic-description parent-design) ,verb-modifier ,answer] stream))
...
(defparameter *non-implementable-built-ins* 
    '(take put index-enumerator list-enumerator vector-length vector-push vector-accessor allocate state-source state-sink print))
...
(defun generate-code (top-level-task &optional (build-map t))
  (if build-map
  ...
  (let* ((top-level-arguments (loop for input-port in (inputs top-level-task) collect (name input-port)))
    ...
    (loop for (source source-name code locality) in (Find-state-initializations top-level-task)
	do (push (list source source-name) state-variable-map)
	when (and code (or (not (eql locality 'local)) (eql (abstract-task (superior source)) top-level-task)))
	...
	(let* ((output-name (name output))
      ...
      (let* ((*completed-tasks* nil)
	     (the-code (code-for top-level-task (task-type top-level-task) :lisp)))
	(values
	 (if state-variable-code
	     `(defun ,(name top-level-task) ,top-level-arguments
		 (let ,state-variable-code
		   ,@the-code
		   ,@(unless streaming?
		       `(,@(program-outputs top-level-task)))))
	   `(defun ,(name top-level-task) ,top-level-arguments
	      ,@the-code
	      ,@(unless streaming?
		   `(,@(program-outputs top-level-task))))
...
(defun allocation-code (type)
  ...
  (ask `[allocation-code ,type ?code]
       #'(lambda (just)
	   (declare (ignore just))
	   (return-from allocation-code ?code))))
...
(defmethod propagate ((task primitive-branching-task) (type (eql 'index-enumerator)))
  (let* ((input (first (inputs task)))
...
(defmethod task-has-completed-execution ((task output-side-mixin))
  (loop for p in (outputs task)
      ...
      do (loop for dataflow in (outgoing-flows p)
	     for destination-port = (output dataflow)
	     do (set-port-symbolic-token destination-port token)))
...
(defmethod code-for :around ((task t) (task-type t) (language t))
  (let* ((upstream-tasks (upstream-tasks task))
	 (entry-condition (and (loop for task in upstream-tasks
				   always (or (typep task 'state-source) (member task *completed-tasks*)))
...
(defmethod code-for :around ((task task-interface-mixin) (task-type t) (language (eql :lisp)))
  (let ((answer (call-next-method))
	...
	(loop for (source source-name code locality) in (Find-state-initializations task)
	    when (and code (eql locality 'local) (eql (abstract-task (superior source)) task))
...
(defmethod generate-opaque-task-code ((task task-interface) language)
...
(defmethod code-for ((task implementation) (task-type t) (language t))
  (loop for sub-task in (children task)
      when (typep sub-task 'initial-input)
...
(defmethod code-for ((task primitive-branching-task) (task-type (eql 'index-enumerator)) (language (eql :lisp)))
  (let* ((more-branch (branch-named 'more task))
    ...
    `((loop for ,index below ,upper-bound
	      do ,@more-branch-code
...
(defmethod code-for ((task primitive-branching-task) (task-type (eql 'list-enumerator)) (language (eql :lisp)))
  (let* ((more-branch (branch-named 'more task))
	 (more-output-port (first (outputs more-branch)))
	 (more-output-port-token (symbolic-token more-output-port))
	 (element (value more-output-port-token))
	 (the-list-token (the-list more-output-port-token))
	 (the-list (value the-list-token))
	 (more-branch-code (code-for more-branch (task-type more-branch) language))
	 (empty-branch (branch-named 'empty task))
	 (empty-branch-code (code-for empty-branch (task-type empty-branch) language)))
...
(defmethod code-for ((task state-sink) (task-type (eql 'state)) (language (eql :lisp)))
  (let* ((state-name (state-name task))
	 ...
	 (new-value-code (code new-value-token)))
...
(defmethod upstream-tasks ((task input-side-mixin))
  (append (loop for input in (inputs task)
	      for token = (symbolic-token input)
...
(defmethod upstream-tasks ((task joining-task))
  (let ((answer nil))
	...
	do (loop for task in (upstream-tasks join)
...
(defun process-component-description (form parent-name)
  (destructuring-bind (&rest plist &key type name &allow-other-keys) form
    (clim-utils:with-keywords-removed (property-plist plist '(:name :type))
